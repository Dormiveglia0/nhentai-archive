...
- Implemented and refit Phase 2 dictionary foundation:
  ...
  - Dictionary UI fourth pass (layout + polish per live feedback): replaced the equal three-column split (which crowded the candidate list) with a **master–detail layout** — wide candidate pool on the left (`.candidate-pool`), editor + evidence stacked on the right inside `.dictionary-detail` (2-col grid `minmax(380px,0.82fr) minmax(440px,1.18fr)`). Summary strip switched back to full-width even distribution with vertical hairline dividers (A-style) instead of left-packed. Candidate status is now color-coded text (B-style: 已配置 green / 待复核 amber / others muted), dropping the dot. Candidate table got a slim custom scrollbar. Buttons restyled: solid `--surface-solid` fill (not muddy translucency), 8px radius, real hover states, primary = solid terracotta, danger = ghost. This "minimal + premium, master–detail" language is the current dictionary baseline.
