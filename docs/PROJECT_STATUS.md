...
- Implemented and refit Phase 2 dictionary foundation:
  ...
  - IMPORTANT layout decision: a master–detail experiment (stacking editor + evidence on the right) was rejected by the user as too far from `design/词典.png`. The dictionary MUST stay the three-column workspace from the design figure: `候选术语池 | 术语编辑器 | 证据面板` side-by-side, equal-height (`grid-template-columns: minmax(320px,1fr) minmax(360px,1.06fr) minmax(300px,0.94fr)`, `align-items: stretch`). Do not move the evidence panel out of its column or restructure the three-column layout. Match the design figure; do not invent new layouts.
