...
- Implemented and refit Phase 2 dictionary foundation:
  ...
  - Dictionary UI fifth pass (user-directed layout, current baseline): top workspace is two columns `候选术语池 | 术语编辑器` (`.dictionary-workspace` grid `minmax(360px,1fr) minmax(420px,1.04fr)`, equal-height). The evidence panel and the old apply-preview tray were **merged** into one full-width `证据与应用预览` panel below the workspace (`DictionaryEvidencePanel` now has tabs 应用影响/关联作品/搭配与标签/远端信息/历史记录; the 应用影响 tab renders the preview impact). 批量导入 is no longer a bottom panel — it is a small button in the candidate-pool header that opens a modal (`.dictionary-modal`, reuses `.preview-backdrop`, Escape/backdrop close). Deleted `DictionaryApplyPreview.tsx` and its dead CSS. Rationale (user): the design figure's bottom is the apply-preview (not bulk import), and the right evidence panel overlapped it, so they were fused; bulk import belongs in a popup. Keep this layout; do not move bulk import back to a bottom panel or split evidence+preview again.
