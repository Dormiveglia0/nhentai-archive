...
运行版本 `3de28cfb7d4557496769ff63378164d2f32797d9` 已推送 `codex/rhine-frontend` 并部署到 http://38.58.176.50:4349/ ，未合并 main（仍为 c41be80）。镜像 `sha256:fdc88100af32376fbb47b5c7b80b5350e59cff35567f605bc555313fc03959bf`，容器 healthy。公网验证健康接口、未登录内容隔离、1440/390居中登录布局及实际新构建共享/页面CSS通过；旧镜像保留为 `nh-archive:rollback-94e5364`，原始远端留档 `2b4fbef201ff6140e3ed7d0878fb5ddbeb60901d` 保留。

交付范围：库、队列、文件、导出主体在 f4ea840；治理与词典在 a36e18a；首页、热门、登录与二级页面在 a488128；首页绘制优化在3de28cf。设置、阅读器底部控件和库详情来源浮层保留。38项关联回归通过，最后首页绘制调整后3项定向复测通过；Docker构建通过。继续阅读手机不显示独立tag资料，桌面以真实标签为主。

验证边界：交互检查使用真实数据的隔离副本，未执行真实删除、导出下载或源文件回写；运行中多任务缺少实际样本。公网检查不绕过认证，登录后的完整行为在隔离环境核验。本机无头Chromium采样表明绘制开销改善，不宣称用户设备恒定60fps。下一阶段仅依据实际使用反馈定位问题，不再自动重做已保留的设置与阅读器控件。
...
验证：Web build 与36项跨页面回归通过；二级页面2项检查通过（首次测试误读详情接口为result已修正）。登录7项覆盖短屏、四尺寸过渡落位及减少动态效果；历史/详情1440、2560、390截图无横向溢出与pageerror。手机续读无独立tag区，桌面tag筛选保留。2560×1440无头Chromium三轮导航/滚动采样：去掉首页重复模糊阴影及多余preserve-3d后，首页>34ms帧31→3、最慢帧76→47ms，12段采样无脚本长任务和pageerror，顶栏始终y=0/64px；减少动态效果时无运行中的路径动画。该采样不是用户设备的性能保证。正式部署结果另记。无API/schema变更，无假作品或统计。设置、阅读器底部控件、库详情来源浮层保留。
