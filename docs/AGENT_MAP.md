...
| --- | --- | --- | --- | --- | --- |
| 首页（#workbench） | `demo/modules/WorkbenchDemo.tsx` | `folio/ui/HomeHero.tsx` | `folio/ui/HomeHero.css`, `workbench/WorkbenchPage.css` | `workbench/WorkbenchPage.tsx` | `api.librarySearch({ per_page: 36, sort: "recent_added" })` |
...
Homepage is an interactive cover wall / 3D browsing stage in HomeHero. It fetches up to 36 real works; pointer drag, wheel, directional buttons and keyboard pan the wall, selection morphs the same covers into a depth carousel. No headline, counters, shelves, dashboard or duplicate navigation. Brand icon: apps/web/public/icon.svg, linked by apps/web/index.html. Do not restore the old dashboard ledger or title descriptions.
