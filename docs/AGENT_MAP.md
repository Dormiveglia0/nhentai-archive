...
| --- | --- | --- | --- | --- | --- |
| 首页（#workbench） | `demo/modules/WorkbenchDemo.tsx` | `folio/ui/HomeHero.tsx` | `folio/ui/HomeHero.css`, `workbench/WorkbenchPage.css` | `workbench/WorkbenchPage.tsx` | `api.librarySummary()`, `api.libraryStatistics(30)` |
...
Homepage now uses an interactive paper/ink SVG installation in HomeHero: three morphing forms, rotation, spread, print action and playback controls, with real summary and reading statistics. It does not fetch covers. No large headline, welcome copy, repeated navigation or invented metrics. Brand icon: apps/web/public/icon.svg.
