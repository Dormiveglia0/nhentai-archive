...
Use this prompt after the feature-complete local loop remains green (`pytest` + `npm run build`). The latest closure baseline is 212 backend tests plus the route-split frontend build; the 2026-07-16 visual baseline covers 10 core entries at 1440×1000 and 390×844, six interaction paths, reduced-motion behavior, and the TaskDock slow-response regression.
