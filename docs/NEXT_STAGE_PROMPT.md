...
- Preserve reader accounting boundaries: progress writes never create visits; one idempotent local `reading_sessions` row creates one `reading_history` open; only visible foreground time is accumulated monotonically, hidden/background time and remote reading are excluded, and no historical time is fabricated. Session-key generation must work outside secure contexts. Statistics use the browser timezone for local calendar days, compare equal-length periods, keep collection author/Tag distributions independent from favorites, and may attribute a session to its start day until a demonstrated need justifies midnight splitting.
