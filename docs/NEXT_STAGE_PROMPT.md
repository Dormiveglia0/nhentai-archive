...
The 2026-09-06 UI update preserves native link behavior in all four library/workbench shelves. Do not cover shelf artwork with hover action labels or rotate it on hover; use a subtle border and retain reading progress. Pointer capture must begin only after a real mouse drag; touch scrolling and keyboard/modifier clicks remain native. The 2026-09-07 correction restores the original Folio login wake animation: focus registration, scan convergence to the topbar and app reveal; no standalone card, welcome slogans or book illustrations. User-facing copy must describe reader actions and outcomes, not development guarantees such as “真实数据” or implementation notes. Login styling is owned by `AuthWakeDemo.css`; do not restore the deleted `AuthGate.css`. The decorative pre-auth shell must remain noninteractive and data-free. Preserve spring number counting, navigation layout springs (`domMax`), 50ms list staggering, horizontal page transitions, offscreen scene pause and mobile navigation focus containment. Motion verification must inspect intermediate frames, not only the final screenshot. Run `e2e/auth-gate.spec.ts`, `e2e/auth-wake-demo.spec.ts` and `e2e/shelf-navigation.spec.ts`; the shelf tests need an authenticated real archive copy via `E2E_BASE_URL` and `E2E_STORAGE_STATE`.
