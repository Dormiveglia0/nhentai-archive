...
class JobService:
    ...
    def list(self) -> list[dict[str, Any]]:
        ...
        jobs = [self._map(row, include_meta=False) for row in rows]
        work_ids = sorted({value for job in jobs if (value := _as_int(job["target"].get("work_id"))) is not None})
        gallery_ids = sorted({value for job in jobs if (value := _as_int(job["target"].get("gallery_id"))) is not None})
        works: dict[int, dict[str, Any]] = {}
        galleries: dict[int, dict[str, Any]] = {}
        if work_ids:
            placeholders = ",".join("?" for _ in work_ids)
            works = {
                int(row["id"]): row
                for row in self.db.fetchall(
                    f"SELECT id, title, pretty_title, page_count, cover_path FROM works WHERE id IN ({placeholders})",
                    work_ids,
                )
            }
        if gallery_ids:
            ...
            galleries = {
                int(row["gallery_id"]): row
                for row in self.db.fetchall(
                    f"SELECT gallery_id, payload_json FROM remote_galleries WHERE gallery_id IN ({placeholders})",
                    gallery_ids,
                )
            }
        for job in jobs:
            job["meta"] = self._resolve_meta(job["target"], works, galleries)
        return jobs

    def recover_interrupted(self) -> int:
        """Turn process-owned in-flight jobs into retryable terminal records on startup."""
        with self.db.connect() as conn:
            rows = conn.execute(
                "SELECT id, status FROM jobs WHERE status IN ('queued', 'running', 'cancelling')"
            ).fetchall()
            for row in rows:
                job_id = int(row["id"])
                if row["status"] == "cancelling":
                    conn.execute(
                        "UPDATE jobs SET status='cancelled', stage='cancelled', error=NULL, "
                        "retry_after=NULL, updated_at=CURRENT_TIMESTAMP WHERE id=?",
                        (job_id,),
                    )
                    self._log_with_conn(conn, job_id, "info", "应用重启，任务取消已完成")
                else:
                    message = "应用重启导致任务中断，可从任务中心重试。"
                    conn.execute(
    ...
    def _map(self, row: dict[str, Any], include_meta: bool = True) -> dict[str, Any]:
        try:
        ...
        return {
            ...
            "target": target,
            "meta": self._resolve_meta(target) if include_meta else None,
            "error": row["error"],
    ...
    def _resolve_meta(
        self,
        target: dict[str, Any],
        works: dict[int, dict[str, Any]] | None = None,
        galleries: dict[int, dict[str, Any]] | None = None,
    ) -> dict[str, Any] | None:
        ...
        if work_id is not None:
            work = works.get(work_id) if works is not None else self.db.fetchone(
                "SELECT title, pretty_title, page_count, cover_path FROM works WHERE id = ?", (work_id,)
            )
            if work:
                return {
        ...
        if gallery_id is not None:
            cached = galleries.get(gallery_id) if galleries is not None else self.db.fetchone(
                "SELECT payload_json FROM remote_galleries WHERE gallery_id = ?", (gallery_id,)
            )
            if cached:
                try:
