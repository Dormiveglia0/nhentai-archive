...
def map_gallery_summary(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    nested_thumbnail = thumbnail if isinstance(thumbnail, dict) else {}
    return {
        "remote": "nhentai",
        "gallery_id": payload["id"],
        "media_id": payload.get("media_id"),
        "title": payload.get("english_title") or title.get("english") or "Untitled",
        "title_japanese": payload.get("japanese_title") or title.get("japanese"),
        "pretty_title": title.get("pretty"),
        "thumbnail": {
            "path": nested_thumbnail.get("path") if nested_thumbnail else thumbnail,
            "width": payload.get("thumbnail_width") or nested_thumbnail.get("width"),
            "height": payload.get("thumbnail_height") or nested_thumbnail.get("height"),
        },
...
class NhentaiClient:
    ...
    def gallery(self, gallery_id: int, include: str | None = None, fresh: bool = False) -> dict[str, Any]:
        params = {"include": include} if include else None
        return self._get(f"/api/v2/galleries/{gallery_id}", params, fresh=fresh)
    ...
    def download_file(self, url: str, destination) -> None:
        ...
        try:
            ...
            raise NhentaiApiError(502, "network_error", f"Remote download failed: {exc.reason}") from exc

    def _get(self, path: str, params: dict[str, Any] | None = None, fresh: bool = False) -> Any:
        return self._request("GET", path, params=params, fresh=fresh)
    ...
    def _request(
        ...
        try:
            ...
            raise NhentaiApiError(502, "network_error", f"Remote request failed: {exc.reason}") from exc
        except (UnicodeDecodeError, json.JSONDecodeError) as exc:
            raise NhentaiApiError(502, "invalid_response", "Remote API returned invalid JSON.") from exc
