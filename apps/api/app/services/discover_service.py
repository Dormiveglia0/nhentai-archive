...
class DiscoverService:
    ...
    def gallery(self, gallery_id: int) -> dict[str, Any]:
        ...
        related_work_ids = self._imported_work_ids(related_items)
        related = [self._with_import_state(item, related_tag_map, related_work_ids) for item in related_items]
        return {
            "remote": "nhentai",
    ...
    def _map_items(self, items: list[dict[str, Any]], meta: dict[str, Any]) -> dict[str, Any]:
        ...
        work_ids = self._imported_work_ids(items)
        return {
            "result": [self._with_import_state(item, tag_map, work_ids) for item in items],
        } | meta

    def _with_import_state(
        self,
        item: dict[str, Any],
        tag_map: dict[int, dict[str, Any]] | None = None,
    ...
    def _imported_work_ids(self, items: list[dict[str, Any]]) -> dict[int, int]:
        gallery_ids = list(dict.fromkeys(int(item["id"]) for item in items if isinstance(item.get("id"), int)))
        if not gallery_ids:
            return {}
        placeholders = ",".join("?" for _ in gallery_ids)
        return {
            int(row["remote_gallery_id"]): int(row["id"])
            for row in self.db.fetchall(
                f"SELECT id, remote_gallery_id FROM works WHERE remote_gallery_id IN ({placeholders})",
                gallery_ids,
            )
        }
