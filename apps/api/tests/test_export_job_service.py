...
def test_mark_downloaded_never_deletes_path_outside_job_directory(tmp_path):
    _settings, _db, _archive, _exports, jobs, export_jobs = _setup(tmp_path)
    outside = tmp_path / "keep.txt"
    outside.write_text("keep")
    job = jobs.create("bulk_export", {"artifact_path": str(outside)})
