...
def test_corrupt_active_job_target_does_not_break_file_scan(tmp_path):
    settings, db, archive, files = _setup(tmp_path)
    _import_work(db, archive, tmp_path)
    (settings.tmp_dir / "partial.download").write_bytes(b"old")
    db.execute(
        "INSERT INTO jobs (type, status, stage, target_json) "
        "VALUES ('remote_import', 'running', 'downloading_cbz', '[')"
    )

    result = files.inventory(category="stale")

    assert [entry["name"] for entry in result["result"]] == ["partial.download"]
...
def test_inventory_batches_tags_for_visible_works(tmp_path, monkeypatch):
    _settings, db, archive, files = _setup(tmp_path)
    first = _import_work(db, archive, tmp_path, title="First", gallery_id=1)
    second = _import_work(db, archive, tmp_path, title="Second", gallery_id=2)
    db.execute(
        "INSERT INTO remote_tags (remote_id, type, name, slug, payload_json) "
        "VALUES (10, 'tag', 'rain', 'rain', '{}'), (11, 'tag', 'snow', 'snow', '{}')"
    )
    db.execute(
        "INSERT INTO work_tags (work_id, remote_tag_id, tag_type, remote_name) "
        "VALUES (?, 10, 'tag', 'rain'), (?, 11, 'tag', 'snow')",
        (first, second),
    )
    ...
    def tracked_fetchall(sql, params=()):
        if "FROM work_tags wt" in sql:
            tag_queries.append(sql)
        return original_fetchall(sql, params)

    monkeypatch.setattr(db, "fetchall", tracked_fetchall)

    result = files.inventory(category="work")["result"]

    assert {entry["work_id"]: entry["tags"] for entry in result} == {
        first: ["rain"],
        second: ["snow"],
    }
