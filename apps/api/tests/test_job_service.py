...
def test_job_list_batches_metadata_queries_and_tolerates_corrupt_targets(tmp_path, monkeypatch):
    jobs = _service(tmp_path)
    jobs.db.execute(
        "INSERT INTO works (id, title, source, page_count, cover_path) VALUES (7, 'Local', 'local', 8, '/cover.jpg')"
    )
    jobs.db.execute(
        "INSERT INTO remote_galleries (gallery_id, payload_json) VALUES (9, ?)",
        (json.dumps({"title": {"pretty": "Remote"}, "num_pages": 12}),),
    )
    local = jobs.create("remote_import", {"work_id": 7})
    remote = jobs.create("remote_import", {"gallery_id": 9})
    corrupt = jobs.create("remote_import", {"gallery_id": 10})
    jobs.db.execute("UPDATE jobs SET target_json = '[' WHERE id = ?", (corrupt["id"],))
    monkeypatch.setattr(
        jobs.db,
        "fetchone",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(AssertionError("list must batch metadata")),
    )

    listed = {job["id"]: job for job in jobs.list()}

    assert listed[local["id"]]["meta"]["title"] == "Local"
    assert listed[remote["id"]]["meta"]["title"] == "Remote"
    assert listed[corrupt["id"]]["target"] == {}
    assert listed[corrupt["id"]]["meta"] is None


def test_recover_interrupted_jobs_keeps_paused_and_closes_process_owned_states(tmp_path):
    jobs = _service(tmp_path)
    queued = jobs.create("remote_import", {"gallery_id": 1})
    running = jobs.create("remote_import", {"gallery_id": 2})
    jobs.mark_running(running["id"], "downloading_cbz")
    cancelling = jobs.create("remote_import", {"gallery_id": 3})
    jobs.cancel(cancelling["id"])
    paused = jobs.create("remote_import", {"gallery_id": 4})
    jobs.mark_running(paused["id"], "downloading_cbz")
    jobs.pause(paused["id"])

    recovered = jobs.recover_interrupted()

    assert recovered == 3
    assert jobs.get(queued["id"])["status"] == "failed"
    assert jobs.get(running["id"])["status"] == "failed"
    assert jobs.get(cancelling["id"])["status"] == "cancelled"
    assert jobs.get(paused["id"])["status"] == "paused"
    assert "应用重启" in jobs.logs(running["id"])["result"][-1]["message"]
