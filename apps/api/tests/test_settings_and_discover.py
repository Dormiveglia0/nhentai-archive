...
def test_discover_feed_batches_import_state_lookup(tmp_path, monkeypatch):
    settings = Settings(data_dir=tmp_path / "data", database_path=tmp_path / "data" / "archive.db")
    db = Database(settings.database_path)
    db.init_schema()
    db.execute(
        "INSERT INTO works (id, remote_gallery_id, title, source) VALUES (7, 101, 'Imported', 'remote')"
    )
    client = FakeDiscoverClient()
    client.latest = lambda page, per_page: {
        "result": [
            {"id": 101, "title": {"english": "Imported"}, "thumbnail": {"path": "a.jpg"}},
            {"id": 102, "title": {"english": "New"}, "thumbnail": {"path": "b.jpg"}},
        ],
        "num_pages": 1,
        "per_page": per_page,
        "total": 2,
    ...
    monkeypatch.setattr(
        db,
        "fetchone",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(AssertionError("feed must batch import state")),
    )

    payload = service.latest(page=1, per_page=24)

    assert [(item["gallery_id"], item["work_id"]) for item in payload["result"]] == [
        (101, 7),
        (102, None),
    ]
