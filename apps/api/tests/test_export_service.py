...
def test_queue_summary_counts_ready_blocked_and_warning_items(tmp_path, monkeypatch):
    _settings, db, archive, exports = _setup(tmp_path)
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("queue must use the aggregate query")
        ),
    )
    ...
    assert exports.summary() == queue["summary"]
