  ...
  const cover = (
    <div className="folio-governance-work-cover">
      {aggregate.work.cover_path ? (
        ...
        <span className="folio-cover-fallback">暂无封面</span>
      )}
    </div>
  );
  ...
  return (
    <header className={`folio-governance-work tone-${tone}`}>
      ...
      <div className="folio-governance-work-copy">
        ...
        {aggregate.work.title_japanese && aggregate.work.title_japanese !== workTitle(aggregate.work) ? <p>{aggregate.work.title_japanese}</p> : null}
