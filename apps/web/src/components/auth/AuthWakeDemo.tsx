...
export function AuthWakeDemo({ children, preview = false }: Props) {
  ...
  useEffect(() => {
    ...
    if (preview) document.title = "NH Archive · 界面唤醒演示";
    return () => {
  ...
  return (
    <main
      className={`folio folio-no-command auth-wake-demo auth-wake-${phase}${focused ? " is-focused" : ""}${realApp ? " has-real-app" : ""}`}
      aria-label={preview ? "界面唤醒登录概念演示" : "NH Archive 访问控制"}
    ...
    >
      ...
      {!realApp ? <header className="auth-wake-topbar" aria-hidden={!awake}>
        ...
        <nav className="auth-wake-nav" aria-label="休眠导航预览">
      ...
      {phase === "loading" ? null : <section className="auth-wake-access" aria-label="访问控制">
        <header>
          <span>访问控制</span>
