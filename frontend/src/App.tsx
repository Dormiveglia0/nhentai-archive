import { AlertTriangle, Archive, BookOpen, Check, Clock, Database, FileArchive, FileText, Folder, Import, ListTodo, Tags, X } from 'lucide-react';
...
export function App() {
  ...
  async function uploadArchive(file: globalThis.File) {
    const form = new FormData();
    form.append('file', file);
    const headers = new Headers();
    if (token) headers.set('Authorization', `Bearer ${token}`);
    const response = await fetch('/api/library/upload', { method: 'POST', headers, body: form });
  ...
  return (
    <div className="app">
      ...
      <main className="main">
        ...
        {!readerId && view === 'library' ? <LibraryPage state={state} selected={selectedWork} onSelect={chooseWork} onRead={(work) => setReaderId(work.id)} onMetadata={(work) => chooseWork(work, 'metadata')} onUpload={uploadArchive} /> : null}
...
function DashboardPage({ state, onView, onWork }: { state: AppState; onView: (view: View) => void; onWork: (work: Work) => void }) {
  return <Page><PageHero title="工作台" seal="档案" desc="快速进入馆藏、远端发现、任务处理与系统维护。" /><StatRail items={[{ icon: <BookOpen />, label: '总收藏', value: state.works.length, hint: '本地 CBZ' }, { icon: <Clock />, label: '阅读中', value: state.works.filter((w) => w.status === 'reading').length, hint: '继续阅读' }, { icon: <Tags />, label: '词典词条', value: state.dictionary.length, hint: '已入库' }, { icon: <ListTodo />, label: '运行任务', value: state.tasks.filter((t) => t.status === 'running').length, hint: '队列健康' }, { icon: <Database />, label: '导出记录', value: state.exports.length, hint: 'CBZ' }]} /><section className="dashboard-grid"><Panel title="最近入库" action={<button onClick={() => onView('library')}>打开我的库</button>}>{state.works.length ? <div className="mini-shelf">{state.works.slice(0, 5).map((work) => <button key={work.id} onClick={() => onWork(work)}><Cover src={work.cover} title={work.title} /><strong>{work.title}</strong><span>{work.pages}P · {work.language}</span></button>)}</div> : <p>暂无作品。请先上传 CBZ。</p>}</Panel><Panel title="今日任务"><TaskList tasks={state.tasks.slice(0, 5)} /></Panel></section></Page>;
}
...
function LibraryPage({ state, selected, onSelect, onRead, onMetadata, onUpload }: { state: AppState; selected: Work | null; onSelect: (work: Work) => void; onRead: (work: Work) => void; onMetadata: (work: Work) => void; onUpload: (file: globalThis.File) => Promise<void> }) {
  const [uploading, setUploading] = useState(false);
  const [uploadError, setUploadError] = useState('');
  async function submitUpload(event: any) {
    event.preventDefault();
    const file = event.currentTarget.elements.file.files?.[0] as globalThis.File | undefined;
    if (!file) return;
    setUploading(true);
    setUploadError('');
    try {
  ...
  if (!state.works.length || !selected) return <Page><PageHero title="我的库" seal="采藏" desc="这里显示真实后端库数据。当前为空，请上传一个 CBZ 开始。" /><Panel title="上传 CBZ"><form onSubmit={submitUpload}><input type="file" name="file" accept=".cbz,.zip" /><button className="primary" disabled={uploading} type="submit">{uploading ? '上传中...' : '上传'}</button></form>{uploadError ? <p className="login-error">{uploadError}</p> : null}<p>上传成功后会自动刷新作品列表。</p></Panel></Page>;
  return <Page><PageHero title="我的库" seal="采藏" desc="专属的同人志档案馆。" /><section className="library-summary"><span>总收藏 <b>{state.works.length}</b></span><span>阅读中 <b>{state.works.filter((w) => w.status === 'reading').length}</b></span><span>待治理 <b>{state.works.filter((w) => w.status === 'needs_metadata').length}</b></span></section><div className="library-layout"><main className="cover-wall">{state.works.map((work) => <article key={work.id} className={selected.id === work.id ? 'focused' : ''}><button onClick={() => onSelect(work)} onDoubleClick={() => onRead(work)}><Cover src={work.cover} title={work.title} /><strong>{work.title}</strong><small>{work.originalTitle}</small><span>{work.language} · {work.pages}P · {work.source}</span><div>{work.tags.slice(0, 3).map((tag) => <TagChip key={tag}>{tag}</TagChip>)}</div></button></article>)}</main><aside className="inspector library-inspector"><header><span>当前作品</span><button><X size={18} /></button></header><div className="work-line"><Cover src={selected.cover} title={selected.title} small /><div><h3>{selected.title}</h3><p>{selected.originalTitle}</p><span>{selected.language} · {selected.pages}P · {selected.source}</span></div></div><section><h4>标签</h4><div className="tag-row">{selected.tags.map((tag) => <TagChip key={tag}>{tag}</TagChip>)}</div></section><Progress label="阅读进度" value={selected.progress} /><Progress label="元数据完整度" value={selected.metadataScore} /><button className="primary wide" onClick={() => onRead(selected)}>继续阅读</button><div className="split"><button onClick={() => onMetadata(selected)}>编辑元数据</button><button>导出 CBZ</button></div></aside></div></Page>;
}
