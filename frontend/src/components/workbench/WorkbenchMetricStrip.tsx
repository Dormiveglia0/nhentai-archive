...
export function WorkbenchMetricStrip({ overview }: { overview: WorkbenchOverview }) {
  const metrics = [
    ...
    { label: "待治理", value: overview.governance.total, detail: `${overview.governance.missing_metadata} 部缺失元数据`, icon: ClipboardList, tone: overview.governance.total > 0 ? "warn" : "ok" },
