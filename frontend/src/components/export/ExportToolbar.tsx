...
export function ExportToolbar({ query, statusFilter, onQueryChange, onStatusFilterChange, multiSelect, onToggleMultiSelect, onSelectReady, onClear }: {
  ...
  return (
    <section className="folio-export-toolbar">
      ...
      <div className="folio-export-tabs" role="tablist" aria-label="导出状态筛选">
        {STATUS_CHIPS.map(({ value, label }) => (
          <button key={value} type="button" role="tab" aria-selected={statusFilter === value} className={statusFilter === value ? "is-active" : ""} onClick={() => onStatusFilterChange(value)}>
