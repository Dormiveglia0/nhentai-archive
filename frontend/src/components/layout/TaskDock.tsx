import { AlertTriangle, ArrowUpRight, Loader2, RotateCcw, Target } from "lucide-react";
              ...
              return (
                <article className={failed ? "is-failed" : "is-active"} key={job.id}>
                  <span className="folio-task-dock-status" aria-hidden="true">
                    {failed ? <AlertTriangle size={16} /> : <Loader2 className="is-spinning" size={16} />}
                  </span>
                  <span className="folio-task-dock-copy">
                    <strong>{failed ? `${jobTypeLabel(job.type)}失败` : stageLabel(job.stage)}</strong>
